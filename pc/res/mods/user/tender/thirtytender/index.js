require(['base', "trackBase", 'store', 'side', 'juicer', 'header', 'footer', "dialog", 'backTop', 'json', "requirejs"
    ...
    */
    ...
    var sysStatus = [
        ...
        {"status": "SERVICE_EXPIRED_APPLY_QUIT", "value": "已到期", "class": "red"}
    ];
    ...
    function getMyInvestment() {
        $.xxdAjax({
            url: '/tradeCenter/investBiz/myInvestmentProductsByToken/XSCP30T',
            type: 'get',
            ...
            callbacks: function (data) {
                ...
                if ((data.code == "200000") || data.data) {
                    if (data.data.list.length == '0') {
                        $('#incomeTable').append("<tr class='no-record'><td colspan='9'><a href='/detail/thirtytender.html'>当前尚未加入，去看看！</a></td> </tr>")
                        return;
                    }
                    $.each(data.data.list, function (i, item) {
                        thirtytenderStatus = item.status;
                        ...
                        if(item.newStatus == '已到期'){
                            $('#incomeTable').append('<tr>' + '<td>' + '<a href="/detail/thirtytender.html" target="_blank"><span>' + item.name + '</span>' + '<span>' + $.fnDateToString(item.addDate, 'yyyy-MM-dd  HH:mm:ss') + '</span></a>'
                            + '</td>' + '<td>' + item.joinApr + '%</td>' + '<td>' + side.numberFormat(item.account) + '元</td>' + '<td>' + item.period + item.periodUnit + '</td>'
                            + '<td>' + side.numberFormat(item.interest) + '元</td>' + '<td><i class="'+classColor+'">' + item.newStatus + '</i></td>'
                            + '<td class="main-color"><a href="/usercenter/bonds/thirtytender.html?join='+item.joinId+'&thirtytenderStatus='+thirtytenderStatus+'">查看详情</a></td>'
                            + '<td class="main-color"><a href="/commpd/agree/XSCP30T_'+item.productId+'.html?productSign=XSCP30T&productJoinId='+item.joinId+'">查看协议</a></td><td class="main-color"><a href="#" class="J_quitbtn J_quitnormal" value="' + item.joinId + '" alt="' + item.productId + '">申请退出</a></td>'
                            + '</tr>');
                        }else{
                            $('#incomeTable').append('<tr>' + '<td>' + '<a href="/detail/thirtytender.html" target="_blank"><span>' + item.name + '</span>' + '<span>' + $.fnDateToString(item.addDate, 'yyyy-MM-dd  HH:mm:ss') + '</span></a>'
                            + '</td>' + '<td>' + item.joinApr + '%</td>' + '<td>' + side.numberFormat(item.account) + '元</td>' + '<td>' + item.period + item.periodUnit + '</td>'
                            + '<td>' + side.numberFormat(item.interest) + '元</td>' + '<td><i class="'+classColor+'">' + item.newStatus + '</i></td>'
                            + '<td class="main-color"><a href="/usercenter/bonds/thirtytender.html?join='+item.joinId+'&thirtytenderStatus='+thirtytenderStatus+'">查看详情</a></td>'
                            + '<td class="main-color"><a href="/commpd/agree/XSCP30T_'+item.productId+'.html?productSign=XSCP30T&productJoinId='+item.joinId+'">查看协议</a></td><td class="main-color"><a href="#" class="J_quitbtn no-quit">申请退出</a></td>'
                            + '</tr>');
                        }
                    });
                } else {
                                        ...
                                        if (data && data.code == "200000") {
                                            ...
                                            if (data.data.resultCode == 0) {
                                                dialog({
                                                    ...
                                                    "<div class='m-con-bd'>" +
                                                    "<div class='filed-user'>  " +
                                                    "<p style='text-align:center;'>退出成功！</p> </div> " +
                                                    "<div class='filed-user'>  <p class='clearfix'><a class='btn c_close' style='top:0; right:135px;'>确认</a></p> </div>" +
                                                    "</div>"
                                                    + "</div>",
                                                    cancel:function (clo) {
