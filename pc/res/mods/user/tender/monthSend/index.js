    ...
    */
    ...
    function investTable(tableData) {
        var title = '<tr class="title"><th>基本信息</th><th>历史年化收益</th><th>加入金额</th>'
            + '<th>待收收益</th><th>实际收益</th><th>服务期</th><th>到期时间</th><th>状态</th><th>操作</th></tr>';
        var contentArray = [];
        var classColor;
    ...
    function outTable(tableData) {
        var title = '<tr class="title"><th>基本信息</th><th>历史年化收益</th><th>加入金额</th>'
            + '<th>实际收益</th><th>服务期</th><th>到期时间</th><th>状态</th><th>操作</th></tr>';
        var contentArray = [];
        var classColor;
        for (var i = 0, j = tableData.length; i < j; i++) {
            var row = tableData[i];
                ...
                $.xxdAjax({
                    ...
                    callback: function (data) {
                        ...
                        if (exitMatchRecord.total == 0) {
                            $exitMatchRecord.append("<tr class='no-record'><td colspan='8'><a href='/step/stepDetail.html'>当前无退出记录，去看看！</a></td> </tr>");
                        }
                    },
                    error: function () {
                        $exitMatchRecord.append("<tr class='no-record'><td colspan='8'><a href='/step/stepDetail.html'>当前无退出记录，去看看！</a></td> </tr>");
    ...
    var map = {
        ...
        monthPadingUrl: '/tradeCenter/investBiz/myInvestmentProductsByToken/YYP',
        InvestPagination: $(".haveTable-page"),
        exitObject: $("#outTable"),
        exitPadingUrl: '/tradeCenter/investBiz/myInvestmentProductsByToken/YYP',
