define(['base', 'paging', 'store', "md", "laydate", "dialog"], function ($, paging, store, md, laydate, dialog) {
    ...
    var joinId = $.getUrlParam('join', location.search),
        sevengoldStatus = $.getUrlParam('sevengoldStatus', location.search),
        monthgoldStatus = $.getUrlParam('monthgoldStatus', location.search),
        newtenderStatus = $.getUrlParam('newtenderStatus', location.search),
        monthSendStatus = $.getUrlParam('monthSendStatus', location.search),
        goldIngotStatus = $.getUrlParam('goldIngotStatus', location.search),
        stepDetailStatus = $.getUrlParam('stepDetailStatus', location.search),
        thirtytenderStatus = $.getUrlParam('thirtytenderStatus', location.search),
        tabStatus = $.getUrlParam('tab', location.search);
    var sysStatus = [
    ...
    var common = {
        getStatus:function () {
            var sysStatus = [
                {"status": "-1", "value": "出借失败","class":"red"},
                {"status": "0", "value": "出借中","class":"blue"},
                {"status": "1", "value": "出借成功","class":"yellow"},
        ...
        tabs: function (object) {
            ...
            //从充值页面跳转进来
            if(tabStatus == 2){
                $tabsli.removeClass("active").eq(1).addClass("active");
                $tabContents.addClass("hide").eq(1).removeClass("hide");
            }
            //安全设置页面跳转展示不同的tab
            if(tabStatus == 3){
                $tabsli.removeClass("active").eq(2).addClass("active");
                $tabContents.addClass("hide").eq(2).removeClass("hide");
            }
                    ...
                    $.xxdAjax({
                        ...
                        callback: function (data) {
                            ...
                            if (matchDetailRecord.total == 0) {
                                if(sevengoldStatus && (sevengoldStatus == '2' || sevengoldStatus == '3')){
                                    $matchTableRecord.append("<tr class='no-record'><td colspan='7'>无债权匹配记录！</td> </tr>");
                                }else if(monthgoldStatus && (monthgoldStatus == '2' || monthgoldStatus == '3')){
                                    $matchTableRecord.append("<tr class='no-record'><td colspan='7'>无债权匹配记录！</td> </tr>");
                                }else if(newtenderStatus && (newtenderStatus == '2' ||newtenderStatus == '3')){
                                    $matchTableRecord.append("<tr class='no-record'><td colspan='7'>无债权匹配记录！</td> </tr>");
                                }else if(monthSendStatus && (monthSendStatus == '2' ||monthSendStatus == '3')){
                                    $matchTableRecord.append("<tr class='no-record'><td colspan='7'>无债权匹配记录！</td> </tr>");
                                }else if(goldIngotStatus && (goldIngotStatus == '2' ||goldIngotStatus == '3')){
                                    $matchTableRecord.append("<tr class='no-record'><td colspan='7'>无债权匹配记录！</td> </tr>");
                                } else if(stepDetailStatus && (stepDetailStatus == '2' ||stepDetailStatus == '3')){
                                    $matchTableRecord.append("<tr class='no-record'><td colspan='7'>无债权匹配记录！</td> </tr>");
                                }  else if(thirtytenderStatus && (thirtytenderStatus == '2' ||thirtytenderStatus == '3')){
                                    $matchTableRecord.append("<tr class='no-record'><td colspan='7'>无债权匹配记录！</td> </tr>");
                                } else{
