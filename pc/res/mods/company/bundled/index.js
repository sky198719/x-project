require(['base', "trackBase", 'store', 'juicer'
    ...
    if(document.domain == 'stage.xxd.com'){
        domain = 'http://stage-static.xxd.com/pc/1.2.6/build/img/';
    }else if(document.domain == 'dev.xxd.com'){
        domain = 'http://dev-static.xxd.com/pc/1.2.6/build/img/';
    }else if(document.domain == 'uat.xxd.com'){
        domain = 'http://uat-static.xxd.com/pc/1.2.6/build/img/';
    }else if(document.domain == 'www.xinxindai.com'){
        domain = 'http://static.xinxindai.com/pc/1.2.6/build/img/';
    }
    ...
    function cardChange(){
        $.ajax({
            url:'/userCenter/user/changeCard/openFuiouChangeCardPage/staticPc',
            type:'post',
            async:false,
            cache:false,
            beforeSend:function(request){
            ...
            success:function(data){
                if(data.code != '200000'){
                    ...
                    if(data.data.code != 0){
                        ...
                        huarunUrl = data.data.data.fuiou_change_card_page_url;
                        ...
                        dialog({
                            ...
                            "<div class='m-con-bd'>" +
                            "<div class='filed-user'>  " +
                            "<p>更换银行卡完成前请不要关闭此窗口，完成操作后请根据您的情况点击下面的按钮</p> </div> " +
                            "<div class='filed-user'>  <p class='clearfix'><a class='btn_left left c_confirm' id='J_submitApply'>已更换银行卡</a><a class='btn btn_right right c_close' >暂不更换银行卡</a></p> </div>" +
                            "</div>"
                            + "</div>",
                            cancel:function (clo) {
                                clo.close();
