...
require(['base', "trackBase", 'float','store','companyHeader','footer','laydate',"dialog", 'formValidate','json', "requirejs"], function ($,  track,float,store,header,footer, laydate,dialog) {
    ...
    var dataUrl={
        ...
        openFuiouOpenAccountPage:'/userCenter/user/enterpriseCapitalAccountV1/openFuiouOpenAccountPage/staticPc',    //开通存管获取信息
    ...
    var commonFun={
        ...
        showInfo:function(step){
            switch(step){
                case 'first':
                    $('#J_showStepInfo').html('实名认证');
                    break;
                case 'second':
                    $('#J_showStepInfo').html('企业认证');
                    break;
                case 'third':
                    $('#J_showStepInfo').html('银行卡认证');
                    break;
                case 'fourth':
        ...
        success: function (data) {
            ...
            if (companyRealNameApproStatus !== '1') {
                ...
                if (companyRealNameApproStatus === '5') {
                    //  审核中
                    commonFun.showCheckStatus('J_showArtificial', '实名认证审核中');
                    return;
                }
                // 2, 3, 4
            ...
            if (companyBankApproStatus !== '1') {
                ...
                if (companyBankApproStatus === '5') {
                    // 审核中
                    commonFun.showCheckStatus('J_showArtificial', '银行卡认证审核中');
                    return;
                }
                // 2, 3, 4
    ...
    var firstStep={
        enterJudge:function (userName,Idnumber) {
            if(userName=='' || userName=="null"){
                commonFun.showError('J_firstStep1Error','请输入您的真实姓名');
                return false;
            }
            if(Idnumber=='' || Idnumber=="null"){
                commonFun.showError('J_firstStep1Error','请输入您的身份证号码');
                return false;
            }
            return true;
        },
        enterPeopleJudge:function (userName,Idnumber,pic1,pic2) {
            if(userName=='' || userName=="null"){
                commonFun.showError('J_firstStep2Error','请输入您的真实姓名');
                return false;
            }
            if(Idnumber=='' || Idnumber=="null"){
                commonFun.showError('J_firstStep2Error','请输入您的身份证号码');
                return false;
            }
            if(pic1==''){
                commonFun.showError('J_firstStep2Error','请上传您的身份证正面照片');
                return false;
            }
            if(pic2==''){
                commonFun.showError('J_firstStep2Error','请上传您的身份证反面照片');
                return false;
            }
            return true;
        },
...
//第四步企业信息录入
    var fourth={
        ...
        loginCompanyInfo:function () {
            ...
            //     commonFun.showError('J_shareholderError','股东名称不能为空');
            //     result=false
            // }else{
...
//第五步企业存管开户
                    ...
                    if(result.fuiouParams){
                        ...
                        if(result.province_id && dataList.city_id){
                            commonFun.getCity(result.province_id,function (result) {
                                var html = '<option value="100000" selected>请选择省/市</option>';
                                var regionHtml='<option value="100000" selected>请选择市/区</option>';
                        ...
                        //卡号
                        if(dataList.capAcntNo){
                            $('#J_legalBankNumber').val(dataList.capAcntNo);
                        }
                        if(OpenAccount!=1){
    ...
    $(function(){
        ...
        $(document).on('click','.close,#J_cancelBtn,#J_applyBtn',function(){
        ...
        function manualBankStep() {
            ...
            $('#J_select2Bank').val(stepData3.comBankCode);
            $('#J_province2Num').val(stepData3.provinceCode);
            $('#J_city2Num').val(stepData3.cityCode);
            $('#J_bank2Num').val(stepData3.comBankNo);
        }
        function autoBankStep() {
            $('#J_third > .step2').addClass('disnone');
            $('#J_third > .step1').removeClass('disnone');
            $('#J_selectBank').val(stepData3.comBankCode);
            $('#J_provinceNum').val(stepData3.provinceCode);
            $('#J_cityNum').val(stepData3.cityCode);
            $('#J_bankNum').val(stepData3.comBankNo);
        }
        function getCacheData(value ,callback) {
            if(citiesCache[value]) {
                return callback && callback(citiesCache[value]);
            }
            commonFun.myAjax({
                url: dataUrl.fuiouDicCities,
                type: 'GET',
        ...
        // 银行选择事件
        $('#J_selectBank,#J_select2Bank').on('change', function(e) {
            var select = this;
            var bankCode = select.options[select.selectedIndex].value;
            var bankname = select.options[select.selectedIndex].text;
            stepData3.comBankCode = bankCode;
            stepData3.comBankName = bankname;
            if (bankCode === '0313' || bankCode === '0314' || bankCode === '0402') {
                // 其他银行跳转人工审核
                if($('#J_third > .step2').hasClass('disnone')) {
                    manualBankStep();
                }
                $('.bank-name').removeClass('disnone');
            } else {
                $('.bank-name').addClass('disnone');
                if(!bankFallbackFlag) autoBankStep();
            }
        });
        $('#branch-bank-name').on('change', function(e) {
            stepData3.comBranchName = this.value;
        })
        // 省份选择
        $('#J_provinceNum,#J_province2Num').on('change', function(e) {
            var select = this;
            stepData3.provinceCode = select.options[select.selectedIndex].value;
            getCacheData(this.value, function(data){
                var html = '<option value="" disabled selected>请选择地区</option>'
                data.forEach(function(item) {
                    html += ('<option value="' + item.regionCode + '">' + item.regionName + '</option>');
                });
                $('#J_cityNum,#J_city2Num').html(html);
            });
        });
        // 城市选择
        $('#J_cityNum,#J_city2Num').on('change',function(e) {
            var select = this;
            stepData3.cityCode = select.options[select.selectedIndex].value;
        });
        ...
        function ensureStep3(data, $error, callback) {
            if(data.comBankCode === '') {
                $error.removeClass('hidden').html('请选择开户银行');
                return callback && callback(false);
            }
            if(data.cityCode === '') {
                $error.removeClass('hidden').html('请选择城市');
                return callback && callback(false);
            }
            if(data.provinceCode === '') {
                $error.removeClass('hidden').html('请选择省/市');
                return callback && callback(false);
            }
            if(data.comBankNo === '') {
                $error.removeClass('hidden').html('请输入银行卡号');
                return callback && callback(false);
            }
            if(data.comBankNo.length > 30) {
                $error.removeClass('hidden').html('请输入正确的银行卡号');
                return callback && callback(false);
            }
            $.xxdAjax({
        ...
        $('#J_step2BankBtn').on('click',function(e) {
            ...
            ensureStep3(stepData3, $errorMessage, function (checked) {
                ...
                commonFun.myAjax({
                    ...
                    type: 'POST',
                    headers: { token: token },
                    data: JSON.stringify({ data: stepData3 }),
                    contentType: 'application/json; charset=utf-8',
                    success: function(data) {
                        // 下一步
                        commonFun.showCheckStatus('J_showArtificial', '银行卡认证审核中');
