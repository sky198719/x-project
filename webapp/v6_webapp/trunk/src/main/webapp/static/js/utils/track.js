...
(function () {
    ...
    window._xa_config = {
        ...
        url: "",     //采集url:https://www.xinxindai.com/pv.gif
        userid: "",  //用户id
    ...
    function _uGC(l, n, s) {
        ...
        if (i > -1) {
            ...
            if (c.indexOf('#!') > -1)c = c.sustring(0, c.indexOf('#!'));
        }
        c = _uEC(c);
        return c;
    }
    ...
    Tool.request = function (param) {
        ...
        if (json.type.toLowerCase() == "jsonp") {
            ...
            if (json.type.toLowerCase() == "get" && _xa_config.off == 1) {
                request.send(json.data);
            } else if (json.type.toLowerCase() == "post" && _xa_config.off == 1) {
                request.send();
            }
    ...
    Hanlder.websiteHanlder = function (json) {
        ...
        var utm_source = Tool.cookie.getCookie("utm_source");
        var utm_cn = 0;
        var zinfo = _uCInfo();
        var setUtma = true;
        ...
        if (!utm_source) {
          Tool.cookie.setCookie("utm_source", getMyUrlParam(), _xa_config.path, GetDomain(), 365 * 24 * 30);
        }
      return data;
    };
    ...
    Track = function (params) {
        ...
        this.url = (_xa_config.url && _xa_config.url != '') ? _xa_config.url : ('//' + document.domain + '/pv.gif');
    };

    /**
     * 设置XA 收集服务地址
     * 默认为：http://www.xinxindai.com/pv.gif
     * @param url
     */
    ...
    Track.prototype.eventView = function (param) {
        ...
        var json = {
            ...
            value: param.value || "",       //事件价值(可选)：如注册次数、充值金额、购买金额
        ...
        var __utmu = Tool.cookie.getCookie("__utmu");
        model.utmcc = '__utma=' + (__utma == null ? '' : __utma) + ';+__utmb=' + (__utmb == null ? '' : __utmb) + ';+__utmc=' + (__utmc == null ? '' : __utmc) + ';+__utmz=' + (__utmz == null ? '' : __utmz) + ';+__utmx=' + (__utmx == null ? '' : __utmx) + ';+__utmu=' + (__utmu == null ? '' : __utmu);
