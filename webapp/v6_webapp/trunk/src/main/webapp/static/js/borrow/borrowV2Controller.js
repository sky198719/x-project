 ...
 * 借款标列表
 */
define(['js/borrow/borrowListV2View','js/borrow/borrowUtils'], function (BorrowListView,borrowUtils) {
    var pageSize = 10;
    // 加载flag
    var loading = false;
    var borrowTypes = {};
    var borrowCtrl = {
        init: function () {
            var tempbt = GS.getBorrowType();
            var tempArr = [];
            for(var i in tempbt) {
                tempArr.push('"'+tempbt[i].typeCode+'":"'+tempbt[i].shortName+'"');
            }
            var tempStr = '{'+tempArr.join(",") + '}';
            borrowTypes = JSON.parse(tempStr);
            /**
             * 事件定义
             *  pullToRefresh 下拉刷新
             *  infiniteScroll 无限刷新
        ...
        showBorrowDetail: function () {
        	var borrowId = $$(this).attr("data-id");
        	try {
        		//XXD_TRACK._trackEvent({category: "webapp_borrow_in", action: "buy_now", label: "立即抢购", value: "", custval: "" });
        		var apr =  $$(this).attr("data-borrwoApr");
        		var timeLimit = $$(this).attr("data-timeLimit");
        		var borrowType = $$(this).attr("data-type");
        		var name = "散标直投"+borrowId;
            	var category = "散标直投/"+apr+"%/"+timeLimit+"个月/新"+borrowType+"贷";
            	product_click({id:borrowId,name:name,category:category,list:"散标投资列表"});
        	} catch (e) {}
        ...
        infiniteScroll: function () {
            ...
            loading = true;
           //==========================切记此处不能删除============================
            var totalPage = $$("#totalPageV2").val();
            var currentPage = $$("#currentPageV2").val();
        ...
        selectBorrow: function (currentPage, type) {
            req.callJSON({
                ...
                success: function(data){
                    ...
                    $$("#totalPageV2").val(data.borrowsList.totalPage);
                    ...
                    for (var i in list) {
                        ...
                        borrowPercent = borrowPercent.substr(0,borrowPercent.length-1);
                        borrowPercent = Math.floor(borrowPercent);
                        borrowType = borrowTypes[b.TYPECODE];
                        var surplusAccount="";
                        if(b.LACKACCOUNT<10000){
                            surplusAccount = appFunc.fmoney(b.LACKACCOUNT,2) + "元";
                        }else{
                            surplusAccount = appFunc.fmoney(Math.floor(b.LACKACCOUNT/100)/100,2) + "万";
                        }

                        var aprs = new String(b.APR).split('.');
                        borrowList.push({
                            "borrowId": b.BORROWID,
                            "borrowType": borrowType,
                            "borrowApr":b.APR,
                            "name": b.NAME,
                            "apr":"<span class='font-red font24'>"+aprs[0]+"</span><span class='font-red font18'>"+((aprs[1] == undefined) ? "" : ("."+aprs[1]))+"% </span>",
                            ...
                            "dayType": b.dayType,
                            "period": b.PERIOD,
                            "periodunit": borrowUtils.toPeriodunit(b.PERIODUNIT)
                        });
                    }

                    try {
                        req.callGet({
                            url: GC.getHtmlPath() + 'borrow/borrowListItemV2.html?' + GC.getVersion(),
                            dataType: 'text',
        ...
        tenderListBindEvent: function(){
            var bindings = [
                {
                     element: '#tenderListV2 .swipeout-content',
                     event: 'click',
                     handler: borrowCtrl.showBorrowDetail
                 },
                 {
                     element: '.borrowListSwipeoutV2',
                     event: 'click',
                                        ...
                                        success: function (data) {
                                            ...
                                            if (data.resultCode != 0) {
                                                ...
                                                xxdApp.alert('投标成功！', '恭喜',function(){
                                                    //window.location.reload();
                                                    GS.reloadPage("borrow/borrowListV2.html");
                                                });
                                            }
                                        },
