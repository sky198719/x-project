 ...
 * 债权转让列表
 */
define(['js/trade/tradeRequestListView', 'js/trade/tradeRequestListModel'], function (tradeRequestListView, tradeRequestListModel) {

    var pageSize = 10;
    // 加载flag
    var loading = false;
    var tradeRequestListCtrl = {
        init: function () {
            ...
            var bindings = [
                {
                    ...
                    event: 'refresh',
                    handler: tradeRequestListCtrl.pullToRefresh
                },
                {
                    element: '.infinite-scroll',
                    event: 'infinite',
                    handler: tradeRequestListCtrl.infiniteScroll
                }
            ];

            tradeRequestListView.bindEvents({
                    bindings: bindings
                }
            );
            tradeRequestListCtrl.pullToRefresh();
        },
        ...
        pullToRefresh: function () {
            tradeRequestListCtrl.selectTradeRequestList(1, "pull");
            $$("#tradeRequestCurrentPage").val(1);
            // 重置加载flag
            loading = false;
        },
        ...
        infiniteScroll: function () {
            ...
            loading = true;
            //==========================切记此处不能删除============================
            var totalPage = $$("#tradeRequestTotalPage").val();
            var currentPage = $$("#tradeRequestCurrentPage").val();
            ...
            tradeRequestListCtrl.selectTradeRequestList(currentPage, "push");
            $$("#tradeRequestCurrentPage").val(currentPage);
            //==========================切记此处不能删除============================
            setTimeout(function () {
                loading = false;
            }, 1500);
        ...
        selectTradeRequestList: function (currentPage, type) {
            req.callJSON({
                ...
                success:function(data){
                    ...
                    $$("#tradeRequestTotalPage").val(data.pageNums);
                    ...
                    for (var k = 0; k < list.length; k++) {
                        ...
                        tradeRequestList[k] = new tradeRequestListModel({
                            ...
                            "apr": appFunc.fmoney(b.apr, 2),
                            "remainNumber": b.remainNumber,
                            "status": b.status,
                            "rep_fun": appFunc.fmoney(b.rep_fun, 2)
                        });
                    }

                    try {
                        req.callGet({
                            url: GC.getHtmlPath() + 'trade/tradeRequestListItem.html?' + GC.getVersion(),
                            dataType: 'text',
                            success:function(result){
                                tradeRequestListView.showListItem({result: result, tradeRequestList: tradeRequestList, type: type});
                                tradeRequestListCtrl.tenderListBindEvent();
                                // 加载完毕需要重置
        ...
        tenderListBindEvent: function () {
            var bindings = [
                {
                    element: '#tradeRequestList .swipeout-content',
                    event: 'click',
                    handler: tradeRequestListCtrl.showTradeRequestDetail
                },
                {
                    element: '#tradeRequestList .swipeout-actions-right',
                    event: 'click',
                    handler: tradeRequestListCtrl.trdExpressBid
                }
            ];
            tradeRequestListView.bindEvents({
                    bindings: bindings
        ...
        trdExpressBid: function () {
            ...
            req.callJSON({
                ...
                success: function (data) {
                    ...
                    var userAccount = data.defaultAccount.USABLE;
                                                ...
                                                success: function (result) {
                                                    if (result.resultCode == 0) {
                                                        xxdApp.alert(result.msg, '提示',function(){
                                                            GS.reloadPage("trade/tradeRequestList.html?path=trade");
                                                        });
                                                    } else {
                    ...
                    tradeRequestListCtrl.open_popup();
                }
            });
        },
        open_popup: function(){
            var bindings = [
                {
                    ...
                    event: 'click',
                    handler: tradeRequestListCtrl.tenderAgreement
                }
            ];
            tradeRequestListView.bindEvents({
                    bindings: bindings
