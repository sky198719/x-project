...
define(['js/utils/date', 'js/common/ami', 'js/account/openOnAccount'], function (DateHandle, ami, openOnAccount) {
    ...
    var sevenDaysTradeCtrl = {
                ...
                success: function (result) {
                    if (result.code == 200000) {
                       ...
                       $('#v_7dMostTender').val(productInfo.mostInvestAmount);
                        ...
                        $('.7d_closeTerm').html(productInfo.leastPeriod);
                        $('#v_7dApr').val(productInfo.plannedAnnualRate);
                        var aprhtml = productInfo.plannedAnnualRate+"%";
                        if(productInfo.floatingRate != undefined) {
                            aprhtml += " + " + productInfo.floatingRate + "%";
                            $(".thirtyDaysTradeActivity").show();
                        }
                        $("#30d_apr").html(aprhtml);
                        ...
                        $('#thirty_lowestTender').html(productInfo.leastInvestAmount);
                        $('#thirty_mostTender').html(productInfo.investableAmount);
                        $('#thirty_mostTender2').html(productInfo.investableAmount);
        ...
        sevenDaysAgreement: function () {
            req.callGet({
                url: GC.getHtmlPath() + 'newHand/thirtyDaysAgreement.html?' + GC.getVersion(),
                dataType: 'text',
                success: function (result) {
            ...
            req.callJSON({
                ...
                success: function (data) {
                    if (data.resultCode == 0) {
                        ...
                        var apr = $("#v_7dApr").val();
                        var floatApr = $("#v_7dFloatApr").val();
                        var aprHtml = apr + "%";
                        if(undefined != floatApr && floatApr > 0) {
                            aprHtml += " + " + floatApr + "%";
                        }
        ...
        confirmClickEnd: function () {
            ...
            if (!agreement) {
                xxdApp.alert("请先阅读并同意《新手产品服务协议》、《自动配标委托书》！", "抱歉");
                return;
            }
                                ...
                                xxdApp.modal({
                                    ...
                                    buttons: [
                                        ...
                                        },
                                        {
                                            text: '是',
                                            onClick: function() {
                                                GS.loadPage("personal/setPassword.html?path=personal");
                                            }
                                        }
                                    ]
                                });
