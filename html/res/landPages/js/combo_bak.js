...
require(['base', 'float', 'trackBase', 'store', 'swiper2', 'dialog', "requirejs"], function ($, float, track, store, Swiper, dialog) {
    ...
    function fnOperation(data) {
        ...
        if (tradeName != '') {
            ...
            if(tradeName.indexOf(".") > 0){  //ie8会显示2位小数
                tradeName = tradeName.substring(0,tradeName.length-3);
            }
        ...
        //累计注册人数
        var totalUser = getNvalueByCode(array, 'TOTAL_REGISTER_USER');
        if (totalUser != '') {
            totalUser = numberFormat(totalUser);
            totalUser = totalUser.substring(0,totalUser.length-3);
            $("#J_userNum").html(totalUser);
        }
        //质保服务专款余额
    ...
    if ($("#J_wrapBody").attr('data-page') == "sevengold") {
        ...
        $('#J_invest,#J_joinup,#J_sideRegister,.snap-up,#J_getMoney').on('click', function (ev) {
        ...
        if (isLogin()) {
            ...
            $("#J_invest").attr('href', dataUrl.qtdsUrl);
            //侧边栏
    ...
    if ($("#J_wrapBody").attr('data-page') == "givedata") {
        ...
        if (isLogin()) {
            if (newTime <= lastTime) {
                $("#J_investl").attr('href', dataUrl.qtdsUrl);
    ...
    if ($("#J_wrapBody").attr('data-page') == "monthgold") {
        mainScroll(500);
        $('#J_invest,#J_bbgs,#J_qtds,#J_xyb,#J_joinup,#J_sideRegister').on('click', function (ev) {
            if (!isLogin()) {
                var ev = ev || event;
                dialogStyle($(this), 271);
                ev.preventDefault();
        ...
        if (isLogin()) {
            ...
            $("#J_qtds").attr({'href': dataUrl.qtdsUrl, 'target': '_blank'});
            $("#J_bbgs").attr({'href': dataUrl.bbgsUrl, 'target': '_blank'});
    ...
    if ($("#J_wrapBody").attr('data-page') == "redpacket") {
        ...
        if (isLogin()) {
            ...
            $("#J_snapTwo").attr({'href': dataUrl.bbgsUrl, 'target': '_blank'});
        }
    }
    //欢乐星期三
    ...
    function mainScroll(h) {
        ...
        function showLighter(slidetop) {
            var content = document.documentElement.scrollHeight || document.body.scrollHeight,
                winH = $(window).height();
        ...
        $.ajax({
            ...
            success: function (result) {
                if (result.code === "200000") {
                    ...
                    $("#J_money").html(numberFormat(result.usableAmount));
