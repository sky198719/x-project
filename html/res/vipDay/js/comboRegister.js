/**
 * Created by gaoshanshan_syp on 2017/11/23.
 */
require(['base', 'float','trackBase', 'store', "dialog", "requirejs"], function ($, float,track, store, dialog) {
                ...
                $.ajax({
                    ...
                    success:function (result) {
                        if (result.regResultCode === '200000') {
                            float.alert({content:'注册成功！'});
                            track.init(result.data.user);
                            setTimeout(function () {
                                // window.location.href="/html/vipDay/Jan.html";
                                window.location.href="/html/vipDay/Feb.html";
       ...
       function dmpGtag() {
           gtag('event', 'purchase', {
               ...
               'transaction_id': '',
               'quantity': '',
               'send_to': 'DC-8281908/sales/s84sbqow+items_sold'
           });
       }
        //判断手机验证码
        function checkPhone(){
