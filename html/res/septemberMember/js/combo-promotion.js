/**
 * Created by gaoshanshan_syp on 2017/9/15.
 */
require(['base', 'float','trackBase', 'store', "dialog", "requirejs", "paging"], function ($, float,track, store, dialog) {
    ...
    //登录成功显示
    if(isLogin()){
       $("#J_registerRecord,#J_registerBottom,#J_leftBtn,#J_rightBrn,#J_specialBtn").attr({'href':'/html/septemberMember/index.html','target':'_blank'});
...
//    注册框判断
    ...
    if(isLogin()){
        if(dmp_urlParam == "" || dmp_urlParam == null) {
            $("#J_bbgsInfo").attr({"href":"/step/stepDetail.html","target":"_blank"});
            $("#J_xybInfo").attr({"href":"/xplan/search/list.html","target":"_blank"});
        }else {
            $("#J_bbgsInfo").attr({"href":"/step/stepDetail.html?xxd_utm_source=" + dmp_urlParam,"target":"_blank"});
