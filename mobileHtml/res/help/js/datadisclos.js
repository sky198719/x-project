require(['base', 'requirejs', 'echarts', 'trackBase', 'store', 'countUp'], function ($, requirejs, echarts, track, store, countUp) {

    fnOperateData();
    // 成交量数据
    var monthData = fnMonthData('MONTH_TURNOVER', new Date().getFullYear());
    fnShowTrade(monthData);
    fnInitSelect(monthData, $('#J_datadisclos_tradeyear'), fnShowTrade, 'MONTH_TURNOVER');
    // 用户相关数据
    fnUserData();
    // 注册用户数据
    var regUserData = fnMonthData('MONTH_NEWUSER', new Date().getFullYear());
    fnShowRegUserData(regUserData);
    fnInitSelect(regUserData, $('#J_datadisclos_useryear'), fnShowRegUserData, 'MONTH_NEWUSER');
    // 借款数据
    fnLoanData();
    ...
    // 显示成交量数量
    function fnShowTrade(data) {
        if (!data) {
            return;
        }
        $('#J_datadisclos_monthdeadline').html('截止日期：' + data.date); // 截止日期
        var xData = new Array(), yData = new Array();
        $.each(data.items, function (i, v) {
            xData.push(v.inforName);
            yData.push(v.nvalue);
        });

        var unit; // 单位
        var unitNumber = 1;
        var minNumber  = Math.min.apply(null, yData); // 求最小值
        if (parseInt(minNumber) < 10000) {
            unit = '元'
        } else if (10000 <= parseInt(minNumber) && parseInt(minNumber) < 100000000) {
            unit       = '万元';
            unitNumber = 10000;
        } else {
            unit       = '亿元';
        ...
        $('.voldata').empty();
        var $charts = echarts.init($('.voldata')[0]);
        $charts.clear();
        // 指定图表的配置项和数据
        option = {
            color  : ['#62adff'],
            ...
            yAxis  : {
                axisLabel: {
                    formatter: function (value) {
                        // 动态单位
                        return parseFloat(value / unitNumber) + unit;
                    }
                },
                axisLine : {
                    show: false
            ...
            tooltip: {
                ...
                formatter      : function (params) {
                    var tipContent = params.name + '月<br/>';
                    tipContent     = tipContent + '金额：￥' + fmoney(params.data, 2);
                    return tipContent;
                },
                extraCssText   : 'box-shadow: 0px 5px 5px #888888'
            }
        };
        $charts.setOption(option); // 使用刚指定的配置项和数据显示图表。
    }

    // 初始化成交量年份下拉选择框
    function fnInitSelect(data, $year, fn, code) {
        if (!data) {
            return;
        }
        $.each(data.year, function (i, v) {
            if (v.year == new Date().getFullYear()) {
                $year.append("<option selected value=" + v.year + ">" + v.year + "</option>");
            } else {
        ...
        $year.bind('change', function () {
            var monthData = fnMonthData(code, $(this).val());
            fn(monthData);
        });
    }

    // 运营数据
    function fnOperateData() {
        $.ajax({
    ...
    function fnOperation(data) {
        ...
        new CountUp($('#J_datadisclos_trade').find('.number')[0], 0, getNvalueByCode(array, 'TOTAL_TRADE'), 2, 1.0, options).start();
        var tradeName = getNameByCode(array, 'TOTAL_TRADE');
        if (tradeName != '') {
            $('#J_datadisclos_trade').find('.describe').html(tradeName);
        }
        new CountUp($('#J_datadisclos_count').find('.number')[0], 0, getNvalueByCode(array, 'TOTAL_COUNT'), 0, 1.0, options).start();
        var countName = getNameByCode(array, 'TOTAL_COUNT');
        if (tradeName != '') {
            $('#J_datadisclos_count').find('.describe').html(countName);
        }
        new CountUp($('#J_datadisclos_investuser').find('.number')[0], 0, getNvalueByCode(array, 'TOTAL_FINANCING_USER'), 0, 1.0, options).start();
        var financingUserName = getNameByCode(array, 'TOTAL_FINANCING_USER');
        if (financingUserName != '') {
            $('#J_datadisclos_investuser').find('.describe').html(financingUserName);
        }
        new CountUp($('#J_datadisclos_financinguser').find('.number')[0], 0, getNvalueByCode(array, 'TOTAL_INVEST_USER'), 0, 1.0, options).start();
        var investUserName = getNameByCode(array, 'TOTAL_INVEST_USER');
        if (investUserName != '') {
            $('#J_datadisclos_financinguser').find('.describe').html(investUserName);
        }

        $('#J_datadisclos_tradedate').html('截止日期：' + data.investUserDate); // 截止日期
        $('#J_datadisclos_servicetime').html(data.time); // 安全运营时间

        new CountUp($('#J_datadisclos_reguser').find('.number')[0], 0, getNvalueByCode(array, 'TOTAL_REGISTER_USER'), 0, 1.0, options).start();
        var reguserName = getNameByCode(array, 'TOTAL_REGISTER_USER');
        if (reguserName != '') {
            $('#J_datadisclos_reguser').find('.describe').html(reguserName);
        }
        new CountUp($('#J_datadisclos_income').find('.number')[0], 0, getNvalueByCode(array, 'TOTAL_INCOME'), 2, 1.0, options).start();
        var incomeName = getNameByCode(array, 'TOTAL_INCOME');
        if (incomeName != '') {
            $('#J_datadisclos_income').find('.describe').html(incomeName);
        }
        new CountUp($('#J_datadisclos_balance').find('.number')[0], 0, getNvalueByCode(array, 'VENTURE_BALANCE'), 2, 1.0, options).start();
        var balanceName = getNameByCode(array, 'VENTURE_BALANCE');
        if (balanceName != '') {
            $('#J_datadisclos_balance').find('.describe').html(balanceName);
        }

        $('#J_datadisclos_operatedate').html('截止日期：' + data.registerUserDate);// 截止日期
    }


    // 月度数据(包括成交量 新增用户数量)
    function fnMonthData(code, year) {
        var data;
        $.ajax({
            url        : '/biz/bulletin/monthData',
            contentType: "application/json",
            dataType   : "json",
            data       : {
                code: code,
    ...
    function fnUserData() {
        $.ajax({
            ...
            success    : function (msg) {
                if (msg && msg.code == 200000) {
                    var data = msg.data;
                    showVotePerson(data.investUserItems);
                    showVoteMoney(data.investMoneyItems);
                    showAge(data.investAgeItems);
                    showSex(data.investGenderItems);
                }
            },
            error      : function (data) {
                $.log(data);
    ...
    // 各投资区间人数占比
    function showVotePerson(data) {
        if (!data) {
            return;
        }

        var seriesData = new Array();
        seriesData.push(getNvalueByName(data, '100万以上'));
        seriesData.push(getNvalueByName(data, '50万-100万'));
        seriesData.push(getNvalueByName(data, '20万-50万'));
        seriesData.push(getNvalueByName(data, '10万-20万'));
        seriesData.push(getNvalueByName(data, '5万-10万'));
        seriesData.push(getNvalueByName(data, '1万-5万'));
        seriesData.push(getNvalueByName(data, '1万以下'));

        var $charts = echarts.init($('#J_datadisclos_voteperson')[0]);
        option      = {
            color : ['#62adff'],
            title : {
                text     : '各投资区间人数占比',
    ...
    function showVoteMoney(data) {
        ...
        var $charts = echarts.init($('#J_datadisclos_vote_money')[0]);
        option      = {
            color : ['#62adff'],
            title : {
                text     : '各投资区间金额占比',
    ...
    function showAge(data) {
        ...
        $.each(data, function (i, v) {
            ...
            legendItem.name = v.inforName;
            legendItem.icon = 'circle';
            legendData.push(legendItem);
            var seriesItem   = new Object();
            seriesItem.value = v.nvalue;
            seriesItem.name  = v.inforName;
            seriesData.push(seriesItem);
        });
        var $charts = echarts.init($('#J_datadisclos_age')[0]);
        option      = {
            title  : {
                text     : '投资人年龄分布',
                left     : 'center',
            ...
            color  : ['#98B8DE', '#9CD8EA', '#B5D37E', '#F8DD9F', '#62ADFF'],
            legend : {
                orient      : 'vertical',
                left        : 'right',
                top         : 'middle',
                ...
                formatter   : function (name) {
                    return name + '  ' + getValue(seriesData, name) + '%';
                },
                selectedMode: false,
                data        : legendData
            },
    ...
    function showSex(data) {
        ...
        var $charts = echarts.init($('#J_datadisclos_sex')[0]);
        option      = {
            title  : {
                text     : '投资人性别分布',
                left     : 'center',
    ...
    // 月度增加人数
    function fnShowRegUserData(data) {
        if (!data) {
            return;
        }
        $('#J_datadisclos_userdeadline').html('截止日期：' + data.date); // 截止日期
        var xData = new Array(), yData = new Array();
        $.each(data.items, function (i, v) {
            xData.push(v.inforName);
            yData.push(v.nvalue);
        ...
        $('.mothdata').empty();
        var $charts = echarts.init($('.mothdata')[0]);
        $charts.clear();
        // 指定图表的配置项和数据
        option = {
            color  : ['#62adff'],
            ...
            tooltip: {
                ...
                formatter      : function (params) {
                    var tipContent = params.name + '月<br/>';
                    tipContent     = tipContent + '注册人数：' + fnumber(params.data) + '人';
                    return tipContent;
                },
                extraCssText   : 'box-shadow: 0px 5px 5px #888888'
            }
    ...
    // 不同行业资金占比
    function fnShowIndustry(data) {
        if (!data) {
            return;
        }
        var legendData = new Array();
        var seriesData = new Array();
        $.each(data.loanIndustryList, function (i, v) {
            var legendItem  = new Object();
            legendItem.name = v.inforName;
            legendItem.icon = 'circle';
            legendData.push(legendItem);
        ...
        var $charts = echarts.init($('#J_datadisclos_industry')[0]);
        option      = {
            title  : {
                text     : '资金投向的行业分布',
                left     : 'center',
            ...
            color  : ['#98B8DE', '#9CD8EA', '#B5D37E', '#F8DD9F', '#EEB29B', '#F2A9B3', '#F0B5CE', '#C4A1CA'],
            legend : {
                orient      : 'vertical',
                left        : 'right',
                top         : 'middle',
    ...
    // 不同期限金额占比
    function fnShowDiffTimeLimit(data) {
        if (!data) {
            return;
        }
        $('#J_datadisclos_industrydate').html('截止日期：' + data.loanIndustryDate); // 截止日期
        var legendData = new Array();
        var seriesData = new Array();
        $.each(data.loanDeadlineList, function (i, v) {
            var seriesItem   = new Object();
            seriesItem.value = v.nvalue;
            seriesItem.name  = v.inforName;
            seriesData.push(seriesItem);
        ...
        // 不同期限标按照期限从小到多排序
        var legendItem0  = new Object();
        legendItem0.name = '1-3个月';
        legendItem0.icon = 'circle';
        legendData.push(legendItem0);
        var legendItem1  = new Object();
        legendItem1.name = '4-6个月';
        legendItem1.icon = 'circle';
        legendData.push(legendItem1);
        var legendItem2  = new Object();
        legendItem2.name = '7-12个月';
        legendItem2.icon = 'circle';
        legendData.push(legendItem2);
        var legendItem3  = new Object();
        legendItem3.name = '12个月以上';
        legendItem3.icon = 'circle';
        legendData.push(legendItem3);


        var $charts = echarts.init($('#J_datadisclos_timelimit')[0]);
        option      = {
            title  : {
                text     : '不同期限标的成交金额分布',
                left     : 'center',
            ...
            color  : ['#98B8DE', '#9CD8EA', '#B5D37E', '#F8DD9F'],
            legend : {
                orient      : 'vertical',
                left        : 'right',
                top         : 'middle',
    ...
    // 资金流向排行榜
    function fnShowRank(data) {
        if (!data) {
            return;
        }
        var seriesData = new Array();
        $.each(data.loanAddressList, function (i, v) {
            var seriesItem   = new Object();
            seriesItem.value = v.nvalue;
            seriesItem.name  = v.inforName;
            seriesData.push(seriesItem);
        ...
        for (var i = 0; i < length - 1; i++) {
            for (var j = 0; j < length - i - 1; j++) {
                if (parseFloat(seriesData[j].value) < parseFloat(seriesData[j + 1].value)) {
        ...
        if (length > 10) {
            for (var i = 0; i < 10; i++) {
                rankData.push(seriesData[0]);
                seriesData.splice(0, 1);
            }
            var rankItem   = new Object();
            rankItem.value = 0;
            rankItem.name  = '其他';
            for (var i = 0; i < seriesData.length; i++) {
                rankItem.value = parseFloat(rankItem.value) + parseFloat(seriesData[i].value);
            }
            rankItem.value = parseFloat(rankItem.value).toFixed(1);
            rankData.push(rankItem);

        } else {
            for (var i = 0; i < seriesData.length; i++) {
                rankData.push(seriesData[i]);
            }
        }
        // 展示
        for (var i = 0; i < rankData.length; i++) {
            $('#J_datadisclos_rank').append('<tr><td>' + rankData[i].name + '</td><td>' + rankData[i].value + '%</td></tr>');
        }
    }
    ...
    function fnShowMap(data) {
        ...
        $('#J_datadisclos_mapdate').html('截止日期：' + data.loanAddressDate); // 截止日期
        var seriesData = new Array();
        $.each(data.loanAddressList, function (i, v) {
            var seriesItem   = new Object();
            seriesItem.value = v.nvalue;
        ...
        // 不同排名区域设定不同颜色
        if (length >= 1) {
            seriesData[0].itemStyle                  = new Object();
            seriesData[0].itemStyle.normal           = new Object();
            seriesData[0].itemStyle.normal.areaColor = "#018EE9";
        }
        if (2 <= length <= 3) {
            for (var i = 1; i < 3; i++) {
                seriesData[i].itemStyle                  = new Object();
                seriesData[i].itemStyle.normal           = new Object();
                seriesData[i].itemStyle.normal.areaColor = "#69AEF8";
            }
        }
        if (4 <= length <= 6) {
            for (var i = 3; i < 6; i++) {
                seriesData[i].itemStyle                  = new Object();
                seriesData[i].itemStyle.normal           = new Object();
                seriesData[i].itemStyle.normal.areaColor = "#8EC4FF";
            }
        }
        if (7 <= length <= 10) {
            for (var i = 6; i < 9; i++) {
                seriesData[i].itemStyle                  = new Object();
                seriesData[i].itemStyle.normal           = new Object();
                seriesData[i].itemStyle.normal.areaColor = "#B0D5FD";
            }
        }
        if (10 <= length) {
            for (var i = 9; i < length; i++) {
                seriesData[i].itemStyle                  = new Object();
                seriesData[i].itemStyle.normal           = new Object();
                seriesData[i].itemStyle.normal.areaColor = "#CEE5FF";
            }
        }
        ...
        var seriesItem                            = new Object();
        seriesItem.value                          = '0';
        seriesItem.name                           = '南海诸岛';
        seriesItem.itemStyle                      = new Object();
        seriesItem.itemStyle.normal               = new Object();
        seriesItem.itemStyle.normal.borderColor   = "#CEE5FF";
        seriesItem.itemStyle.emphasis             = new Object();
        seriesItem.itemStyle.emphasis.borderColor = "#CEE5FF";
        seriesData.push(seriesItem);
        ...
        var oChina = echarts.init(document.getElementById('china'));
        var chinaMap      = {
            title    : {
                show: false
            },
            tooltip  : {
                ...
                formatter      : function (params) {
                    if (params.value) {
                        return '资金流向的地域<br/>' + params.name + '：' + params.value + '%';
                    }
                    return '';
                }
            },
            visualMap: [{
                ...
                max: 50,
                calculable: true,
                color     : ['#0275FD', '#69AEF8', '#8EC4FF', '#B0D5FD', '#CEE5FF']
            }],
            series   : [
                {
                    name     : '金额占比',
        ...
        oChina.setOption(chinaMap);
    }

    //借款信息数据
    function fnFourLoanData(data) {
        if (!data) {
            return;
        }
        $("#J_datadisclos_borrdate").html('截止日期：' + data.loanDate);
        $.each(data.loanItems, function (i, v) {
            //待偿金额
            if (v.code == "UNRECEIVED") {
                $("#J_datadisclos_unreceived").find(".number").html(addCommas(v.nvalue));
                $("#J_datadisclos_unreceived").find(".describe").html(v.inforName);
            }
            //逾期金额
            if (v.code == "OVERDUE_AMOUNT") {
                $("#J_datadisclos_overamunt").find(".number").html(addCommas(v.nvalue));
                $("#J_datadisclos_overamunt").find(".describe").html(v.inforName);
            }
            //金额逾期率
            if (v.code == "AMOUNT_OVERDUE_RATE") {
                $("#J_datadisclos_amountoverrate").find(".number").html(addCommas(v.nvalue));
                $("#J_datadisclos_amountoverrate").find(".describe").html(v.inforName);
            }
            //项目逾期率
            if (v.code == "PROJECT_OVERDUE_RATE") {
                $("#J_datadisclos_projectoverrate").find(".number").html(addCommas(v.nvalue));
                $("#J_datadisclos_projectoverrate").find(".describe").html(v.inforName);
            }
        })
    }
    ...
    function getNameByCode(array, code) {
    ...
    function getNvalueByName(array, name) {
        if (!array || !(array instanceof Array)) {
            return 0;
        }
        var length = array.length;
        for (var i = 0; i < length; i++) {
            if (array[i].inforName == name) {
                return array[i].nvalue;
            }
        }
        return 0;
