require(['base', 'requirejs', 'trackBase'], function ($, requirejs, track) {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (accountCashFree == 0) {
                    ...
                    $("#accountCash").html("每月可免费提现" + accountCashFree + "次，超出将按提现金额的" + accountCashRate + "%收取手续费，每笔最低" + accountCashMinfee + "元，最高" + accountCashMaxfee + "元(当月免费提现次数不可叠加至次月使用)");
    ...
    function borrowDetail(borrowId) {
        $.ajax({
            ...
            dataType: 'json',
            success: function (data) {
                var lowesttender = data.borrow.LOWESTTENDER;
                $("#lowesttender").html(lowesttender);
                ...
                var statusValue = data.borrow.STATUSVALUE;
